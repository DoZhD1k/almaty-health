...
export function BuildingAnalysisModal({ onClose, data, onHospitalClick }: BuildingAnalysisModalProps) {
  const [activeTab, setActiveTab] = useState<"objects" | "districts">("objects");
  ...
  const districtsStats = useMemo(() => {
    ...
    const maxRisk = Math.max(...Object.values(byDist).map((r: any) => r.riskCount), 1);
    return Object.entries(byDist)
      .sort((a: any, b: any) => b[1].riskCount - a[1].riskCount)
      .map(([name, r]: any) => ({ name, ...r, maxRisk }));
  }, [data]);
